...
var shash = {
  ...
  /**
   * Checks if SHash support is ready (WASM Module loaded)
   * @return {Boolean}
   */
  isReady: function() {
    return shash.internal.module !== null;
  },

  /**
   * Initializes a Hashing Context for Hash
   * @param {Number} digest_size the number of bits for the digest size (512, 480, 416, 384, 353, 320, 288, 256, 244, 192, 160 or 128). 512 is default.
  ...
  init: function(digest_size) {
    ...
    if (digest_size < 128 || digest_size > 512 || (digest_size - 128) % 32 !== 0)
      digest_size = 512;

    return {
      'digest_size': digest_size,
      'context': shash.internal.init(digest_size)
    };
  },
  ...
  update: function(contextObject, bytes) {
    var inputBuffer = shash.internal.bufferFromBytes(bytes);

    shash.internal.update(contextObject.context, inputBuffer, bytes.length * 8);

    shash.internal.destroy_buffer(inputBuffer);
  },
   ...
   */
   updateFromValue: function(contextObject, value) {
     shash.update(contextObject, shash.internal.inputToBytes(value));
   },
  ...
  final: function(contextObject) {
    ...
    var digestBuffer = shash.internal.create_buffer(digestByteLen);

    shash.internal.final(contextObject.context, digestBuffer);

    var digestBytes = shash.internal.bytesFromBuffer(digestBuffer, digestByteLen);
    shash.internal.destroy_buffer(digestBuffer);
    return digestBytes;
  },
  ...
  cleanup: function(contextObject) {
    shash.internal.cleanup(contextObject.context);
  },

  /**
   * Calculates the shash message digest ("hash") for the input bytes or string
  ...
  digest: function(input, digest_size) {
    input = shash.internal.inputToBytes(input);

    var ctx = shash.init(digest_size);
    shash.update(ctx, input);
    var bytes = shash.final(ctx);
    shash.cleanup(input);
  ...
  digestHex: function(input, digest_size) {
    var bytes = shash.digest(input, digest_size);
    return shash.internal.toHex(bytes);
  }
};

createSHashModule().then(async module => {
  // Memory allocations helpers
  shash.internal.create_buffer  = module.cwrap('malloc', 'number', ['number']);
  shash.internal.destroy_buffer = module.cwrap('free',   '',       ['number']);

  shash.internal.init    = module.cwrap('shash_init',    'number', ['number']);
  shash.internal.update  = module.cwrap('shash_update',  '',       ['number','number','number']);
  shash.internal.final   = module.cwrap('shash_final',   '',       ['number','number']);
  shash.internal.cleanup = module.cwrap('shash_cleanup', '',       ['number']);
  shash.internal.module  = module;
});
